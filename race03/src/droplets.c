...
int init_droplets(){
    ...
    num_of_droplets = screen_area / 75;
    ...
    for (; i < num_of_droplets; i++){
        gen_droplet_props(droplets);
        (droplets + i)->row =  max_row +  2 * i;
    }
...
int draw_rain(){
    ...
    if(max_col_temp != max_col || max_row_temp != max_row){
        clear();
        if(droplets_realloc(max_col * max_row / 25)){
...
int draw_droplet(struct droplet *drop){
    int temp;
    if(velocity){
        temp = /*drop->length/8 - 2 +*/ refresh_rate;
    }
    else{
        temp = refresh_rate;
    }
...
int droplets_realloc(unsigned int new_num_of_droplets){
    unsigned int temp;
